...
Clone the git repositories for units and gitian and then checkout the units version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the units repository
...
gitian builds' in [doc/release-process.md](release-process.md) in the units repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/units/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/units/units
...
For example:
```bash
URL=https://github.com/konez2k/units.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit units=${COMMIT} --url units=${URL} ../units/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit units=${COMMIT} --url units=${URL} ../units/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit units=${COMMIT} --url units=${URL} ../units/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/units-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/units-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/units-build.assert
```
...
[units/gitian.sigs](https://github.com/units/gitian.sigs/) repository, or if that's not possible to create a pull
request.
